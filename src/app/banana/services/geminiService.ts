...
export async function editImage(
  ...
  try {
    ...
    if (!stellarToken) {
      throw new Error("No stellar token found. Please provide a stellar token in the URL parameters (?stellar=your-token).");
    }
        ...
        try {
            ...
            if (parsedError.error && parsedError.error.message) {
                ...
                if (parsedError.error.status === 'rate_limit_exceeded') {
                    ...
                    errorMessage = "Insufficient quota. Please check your account balance.";
                } else {
